...
工作区根目录执行（无需 ROS、Docker 或 sudo）：

```bash
# 初次复制：保留默认 IP，随后按实际网卡编辑这个副本
bash scripts/init_local_config.sh
# 或创建不同机器的副本，同时设置真实 IP（示例地址请替换）
...
bash scripts/run.sh localization config_file:=config/local/mid360_localization.local.yaml \
  lidar_config:=config/local/MID360.local.json map_path:=pcd_map/实际地图.pcd
```
