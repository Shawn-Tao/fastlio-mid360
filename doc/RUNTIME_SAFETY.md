...
- `max_points=2000000` 控制缓存点数；达上限后拒绝新体素但保留历史区域，告警并
  标记 complete=false。不会静默淘汰旧区域。0 取消上限须同时 static_filter:=false 并自行评估内存。
- `voxel_size=0` 同时 static_filter:=false 可对照旧的逐帧追加机制，仍受容量上限约束。
- 静止重复扫描时点数趋于稳定；噪声、漂移、新区域和动态物体仍可能增加点数。
  complete 只表示没有因确认缓存/候选/帧输入容量截断，并不保证场地覆盖、几何精度或无动态物体。
- 实时 ikd-Tree 不因保存分辨率调整而修改；地图裁剪也只跟随可信位姿。

存图默认启用跨帧确认（至少 3 次、跨度 0.6 s）和自由空间清理（至少 3 次、跨度 0.4 s），
独立计票间隔均为 0.1 s，清理速度/转速上限已放宽为 1.0 m/s、1.0 rad/s。
位置标准差 0.10 m、帧间隔 0.5 s、射线几何保护和预算保持不变，旧本地配置不自动改写。
同帧重复点/射线不重复计票；遮挡、不在视野或未重访不算空闲；只在低速、低转速、
协方差门限内清理，不修改实时树。候选未再次看到 5 s 过期，已确认历史区域永不按年龄淘汰。
候选和端点缓存有界，每帧最多 256 条射线、每条 600 体素步；单帧端点截断时禁止清理。
`/tracking/status` 和地图 JSON 增加 static_map 统计/策略门限；候选不导出或在退出时强行提升。
站定的人仍可能入图，需要人离开后慢速重访才能清理，证据时间门限不代表保证清理时间。
关闭对照 static_filter:=false，参数、细节与动态场景测试见 [静态存图过滤](STATIC_MAP_FILTER.md)。
入图/清理阶段现在支持独立开关和独立计票间隔。次数允许 1、跨度/间隔允许 0，
但仍不把同帧重复点/射线当多次证据，遮挡保护/截断禁止清理保持不变。
行走门控均可配置，/tracking/status 的 archive_clear_gate 提供具体拒绝原因；
放宽清理许可不会消除扫描末端射线原点近似的误差，不保证四足步态下的误删率。
