          ...
          ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          if(((abs(pl_full[i].x - pl_full[i-1].x) > 1e-7)
              ...
              && !is_operator_point(pl_full[i].x, pl_full[i].y, pl_full[i].z))
          {
...
// Points are in the LiDAR body frame: x forward, y left, z up.
// An "operator point" lies inside the rear sector (within rear_angle of the
// backward axis) AND inside the horizontal range band [range_min, range_max].
// Far rear points (walls) are kept so the back hemisphere still constrains
// the scan-to-map match.
bool Preprocess::is_operator_point(float x, float y, float z)
{
  (void)z;
  if (!operator_filter_en) return false;
  if (x >= 0.0f) return false;
  float rear_half_rad = (float)(operator_filter_rear_angle * M_PI / 180.0);
  float azimuth = atan2f(y, x); // for x<0 this lies in (-pi,-pi/2) or (pi/2,pi)
  if (fabsf(azimuth) < (float)M_PI - rear_half_rad) return false;
  float range = sqrtf(x * x + y * y);
  return (range >= (float)operator_filter_range_min && range <= (float)operator_filter_range_max);
}
