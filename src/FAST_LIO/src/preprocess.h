...
//   EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  // Operator (following human) point-cloud filter: drops points inside a rear
  // sector and range band so the operator never enters the map. Enable and use
  // IDENTICAL parameters for map building and localization runs.
  bool operator_filter_en = false;
  double operator_filter_rear_angle = 50.0; // deg, half angle of the rear sector
  double operator_filter_range_min = 0.3;   // m, horizontal range
  double operator_filter_range_max = 3.0;   // m, horizontal range
  bool is_operator_point(float x, float y, float z);
